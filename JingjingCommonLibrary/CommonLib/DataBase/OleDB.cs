...
namespace Jingjing.CommonLib.DataBase
{
    ...
    public class OleDB
    {
        ...
        private OleDbConnection GetConnection(string ConnectionString)
        {
            try
            {
                if (conn == null)
        ...
        public DataTable SelectDataTable(string ConnectionString, string SqlString, OleDbParameter[] OleDbParameters)
        {
            ...
            try
            {
                ...
                if (OleDbParameters != null)
                    foreach (OleDbParameter para in OleDbParameters)
        ...
        /// <param name="OleDbParameters">SQL语句的参数数组</param>
        public void UpdateDataTable(DataTable dt, string ConnectionString, string SqlString, OleDbParameter[] OleDbParameters)
        {
            conn = GetConnection(ConnectionString);
            try
            {
        ...
        public int ExecuteNonQuery(string ConnectionString, string SqlString, OleDbParameter[] OleDbParameters)
        {
            conn = GetConnection(ConnectionString);
            try
            {
        ...
        public object ExecuteScaler(string ConnectionString, string SqlString, OleDbParameter[] OleDbParameters)
        {
            conn = GetConnection(ConnectionString);
            try
            {
        ...
        public void ExecuteOleDbTransaction(string ConnectionString, string[] SqlStrings, OleDbParameter[][] OleDbParameters)
        {
            conn = GetConnection(ConnectionString);
            try
            {
                conn.Open();
                OleDbCommand SqlCmd = conn.CreateCommand();
                ...
                try
                {
                    for (int i = 0; i < SqlStrings.Length; i++)
                    {
                        ...
                        if (OleDbParameters[i] != null)
                            foreach (OleDbParameter para in OleDbParameters[i])
                                SqlCmd.Parameters.Add(para);
